...
export const ScaleAndRadii: Story = {
  render: () => (
    <div style={{ maxWidth: 980, margin: "0 auto", padding: "40px 24px" }}>
      ...
      <p style={{ color: "var(--ink-soft)", maxWidth: "44em", margin: 0 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {scale.map((s) => (
          <div key={s.name} style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <div style={{ width: 90, ...mono }}>{s.name}</div>
            <div style={{ width: 44, ...mono }}>{s.px}px</div>
            <div style={{ height: 14, width: s.px, background: "var(--orange)", borderRadius: 3 }} />
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 18, flexWrap: "wrap" }}>
        {radii.map((r) => (
          <div key={r.name} style={{ textAlign: "center" }}>
            ...
            />
            <div style={{ ...mono, marginTop: 8 }}>{r.name}</div>
            <div style={{ ...mono }}>{r.px}px</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 18, flexWrap: "wrap" }}>
        {[
          ...
          <div key={b.name} style={{ textAlign: "center" }}>
            ...
            />
            <div style={{ ...mono, marginTop: 8 }}>{b.name}</div>
            <div style={{ ...mono }}>{b.px}px</div>
          </div>
        ))}
      </div>
    </div>
